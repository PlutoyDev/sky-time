...
export const getWebhook = (webhook_id: string, lean = true) => {
  return Webhook.findOne({ _id: webhook_id }, null, { lean }).exec();
};

export const getWebhooks = (webhook_ids: string | string[], lean = true) => {
  return Webhook.find({ _id: { $in: webhook_ids } }, null, { lean }).exec();
};
...
export const getGuildWebhooks = (guild_id: string, lean = true) => {
  return Webhook.find({ guild_ids: { $elemMatch: { $eq: guild_id } } }, null, { lean }).exec();
};

export const getMessageWebhook = (message_id: string, lean = true) => {
  return Webhook.findOne({ message_ids: { $elemMatch: { $eq: message_id } } }, null, { lean }).exec();
};
